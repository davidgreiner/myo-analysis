...
def import_myo_csv_file(path,               # Path to CSV File
    ...
    df[' Timestamp'] = df[' Timestamp'].astype('datetime64[ns]') # Set datatype for Timestamp
...
# Labels Data with event_logger txt file
def label_data(df_myo, path, event_file_name): # Myo Dataframe
	file_path = os.path.join(path, event_file_name)
	df_events = pd.read_csv(file_path, sep=': ', header=None, names=[' Timestamp', 'label'])
	df_events = df_events.fillna("other")
	df_events[' Timestamp'] = [datetime.strptime(x, '%Y-%m-%d %H:%M:%S:%f') for x in df_events[' Timestamp']] # Covert Timestamp to Datetime
	df_events[' Timestamp'] = df_events[' Timestamp'].astype('datetime64[ns]') # Set datatype for Timestamp
	df_events = df_events.sort_values(' Timestamp')
	df_myo = df_myo.sort_values(' Timestamp')
	df = pd.merge_asof(df_myo, df_events, on=' Timestamp')
	return df

#def merge_data(df_imu, df_emg):
